...
''' </summary>
''' <remarks>
''' This class originally developed for XSS Tunnel, Modified for BSQL Hacker.
''' </remarks>
...
#Region "Generate Response"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Function GetServerResponse() As Byte()
    ...
    ''' <returns></returns>
    ''' <remarks>
    ''' This is still not safe, because of victim virtually can inject any header so we are totally open to http splitting variations. Eventually it's safe because we are running under victim's credentials so victim will hack himself (in theory). Still not perfect.
    ''' </remarks>
